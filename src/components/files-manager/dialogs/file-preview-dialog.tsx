...
export function FilePreviewDialog({
  ...
  const {
    ...
    queryKey: ["file-preview", file?.slug],
    queryFn: async () => {
      if (!file) return null;
      const res = await fetch(
        `${settings.cdnUrl || ""}/f/${file.slug}/${encodeURIComponent(file.filename)}`,
      );
  ...
  const src = `${settings.cdnUrl || ""}/f/${file.slug}/${encodeURIComponent(file.filename)}`;
