...
export function MultiSelectContent({
  ...
  const { items, onItemAdded, toggleValue, selectedValues } =
    useMultiSelectContext();
  const [query, setQuery] = useState("");

  const normalizedQuery = query.trim();
  // normalize typed value (remove leading dot and lowercase) for comparison/add
  const normalizedForAdd = normalizedQuery.replace(/^\./, "").toLowerCase();
  const canAdd = normalizedForAdd.length > 0 && !items.has(normalizedForAdd);

  function handleAddValue(val: string) {
    const v = String(val).trim();
    if (!v) return;
    const final = v.replace(/^\./, "").toLowerCase();
    // add to internal items store so MultiSelectValue can render it
    onItemAdded(final, final);
    // select it
    if (!selectedValues.has(final)) toggleValue(final);
    setQuery("");
  }
        ...
        <Command {...props}>
          {canSearch ? (
            <CommandInput
              ...
              value={query}
              // command input from cmdk/shadcn uses onValueChange, but forward several handlers
              ...
              onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => {
                if (
                  ...
                  e.preventDefault();
                  handleAddValue(normalizedQuery);
                }
              }}
            />
          ) : (
            ...
            {canSearch && (
              <>
                ...
                {canAdd && (
                  <CommandItem
                    onSelect={() => {
                      handleAddValue(normalizedForAdd);
                    }}
                  >
                    Add "{normalizedForAdd}"
                  </CommandItem>
                )}
              </>
            )}
